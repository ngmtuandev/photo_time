package photo_time_tracking.photo_time.constant;

public class ResourceBundleConstant {
    // Common
    public static final String EM_001 = "EM_001";
    public static final String ERROR_SERVER = "ERROR_SERVER";
    public static final String ERROR_VALIDATION_FIELD = "ERROR_VALIDATION_FIELD";

    // Role
    public static final String RL_001 = "RL_001";
    public static final String RL_002 = "RL_002";
    public static final String RL_003 = "RL_003";
    public static final String RL_004 = "RL_004";
    public static final String RL_005 = "RL_005";
    public static final String RL_006 = "RL_006";

    // User
    public static final String USR_001 = "USR_001";
    public static final String USR_002 = "USR_002";
    public static final String USR_003 = "USR_003";
    public static final String USR_004 = "USR_004";
    public static final String USR_005 = "USR_005";
    public static final String USR_006 = "USR_006";
    public static final String USR_007 = "USR_007";
    public static final String USR_008 = "USR_008";
    public static final String USR_009 = "USR_009";
    public static final String USR_010 = "USR_010";

    // Store
    public static final String STORE_001 = "STORE_001";
    public static final String STORE_002 = "STORE_002";
    public static final String STORE_003 = "STORE_003";
    public static final String STORE_004 = "STORE_004";
    public static final String STORE_005 = "STORE_005";
    public static final String STORE_006 = "STORE_006";
    public static final String STORE_007 = "STORE_007";
    public static final String STORE_008 = "STORE_008";

    // Solution
    public static final String SOLUTION_001 = "SOLUTION_001";
    public static final String SOLUTION_002 = "SOLUTION_002";
    public static final String SOLUTION_003 = "SOLUTION_003";
    public static final String SOLUTION_004 = "SOLUTION_004";
    public static final String SOLUTION_005 = "SOLUTION_005";
    public static final String SOLUTION_006 = "SOLUTION_006";
    public static final String SOLUTION_007 = "SOLUTION_007";
    public static final String SOLUTION_008 = "SOLUTION_008";
    public static final String SOLUTION_009 = "SOLUTION_009";
    public static final String SOLUTION_010 = "SOLUTION_010";
    public static final String SOLUTION_011 = "SOLUTION_011";

    // Error
    public static final String ERROR_001 = "ERROR_001";
    public static final String ERROR_002 = "ERROR_002";
    public static final String ERROR_003 = "ERROR_003";
    public static final String ERROR_004 = "ERROR_004";
    public static final String ERROR_005 = "ERROR_005";
    public static final String ERROR_006 = "ERROR_006";
    public static final String ERROR_007 = "ERROR_007";
    public static final String ERROR_008 = "ERROR_008";
    public static final String ERROR_009 = "ERROR_009";
    public static final String ERROR_010 = "ERROR_010";
    public static final String ERROR_011 = "ERROR_011";
    public static final String ERROR_012 = "ERROR_012";

    // Record Transaction
    public static final String RCTRANS_001 = "RCTRANS_001";
    public static final String RCTRANS_002 = "RCTRANS_002";
    public static final String RCTRANS_003 = "RCTRANS_003";
    public static final String RCTRANS_004 = "RCTRANS_004";
    public static final String RCTRANS_005 = "RCTRANS_005";
    public static final String RCTRANS_006 = "RCTRANS_006";
    public static final String RCTRANS_007 = "RCTRANS_007";
    public static final String RCTRANS_008 = "RCTRANS_008";
    public static final String RCTRANS_009 = "RCTRANS_009";

}
